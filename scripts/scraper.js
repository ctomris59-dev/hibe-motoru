...
const SORGULAR = [
  ...
  'kalkınma ajansı hibe 2026 başvuru açıldı',
...
// 429 rate limit için otomatik retry
async function geminiAraRetry(sorgu, mevcutData, deneme = 1) {
  const MAX_DENEME = 3;
  try {
    return await geminiAra(sorgu, mevcutData);
  } catch (err) {
    const is429 = err.message && err.message.includes('429');
    if (is429 && deneme < MAX_DENEME) {
      const bekleme = deneme === 1 ? 30000 : 60000;
      console.log(`   ⏳ Rate limit — ${bekleme/1000}sn bekleniyor (deneme ${deneme}/${MAX_DENEME})...`);
      await bekle(bekleme);
      return geminiAraRetry(sorgu, mevcutData, deneme + 1);
    }
    throw err;
  }
}
...
async function main() {
  ...
  for (const sorgu of SORGULAR) {
    ...
    await bekle(15000); // Sorgular arası bekleme (rate limit önlemi)
  }
