...
const KAYNAKLAR = [
  ...
  // ── Bölgesel: Trakya / Tekirdağ / Çorlu ────────────────────────────────────
  { ad: 'TRAKYAKA',          url: 'https://news.google.com/rss/search?q=TRAKYAKA+destek+program+2026&hl=tr&gl=TR&ceid=TR:tr' },
  { ad: 'Tekirdağ Hibe',     url: 'https://news.google.com/rss/search?q=Tekirdağ+hibe+teşvik+destek+2026&hl=tr&gl=TR&ceid=TR:tr' },
  { ad: 'Trakya Bölge',      url: 'https://news.google.com/rss/search?q=Trakya+kalkınma+hibe+mali+destek+2026&hl=tr&gl=TR&ceid=TR:tr' },
];

// ── TRAKYAKA doğrudan sayfa kontrolü ─────────────────────────────────────────
// Google News'e ek olarak TRAKYAKA'nın açık programlar sayfasını da kontrol et
...
async function direktSayfaKontrol(kaynaklar) {
  const SNAPSHOT_PATH = require('path').join(__dirname, 'snapshot_direkt.json');
  const fs = require('fs');
  let snapshots = {};
  ...
  for (const k of kaynaklar) {
    try {
      ...
      // Sadece içerik özeti al (menü vs. hariç)
      const metin = body
...
async function main() {
  ...
  // ── AŞAMA 0: TRAKYAKA doğrudan sayfa kontrolü ───────────────────────────────
  const tumBasliklar = [];
